    ...
    return (
        <>
            <div className="md:col-span-2">
                ...
                {variants.map((variant, vIdx) => (
                    <div key={vIdx} className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            />
                            <button onClick={() => setVariants(variants.filter((_, i) => i !== vIdx))} className="ml-2 p-2 text-red-600 hover:bg-red-50 rounded-lg">
                ...
                <button type="button" onClick={() => setVariants([...variants, { name: '', values: [] }])} className="w-full px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-blue-500 hover:text-blue-600 transition-colors flex items-center justify-center gap-2">
                    <Plus className="w-4 h-4" />
                    Add New Variant
                </button>
            </div>
                    ...
                    <div className="space-y-2 mb-3">
                        {features.map((feature, idx) => (
                            <div key={idx} className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg">
                                ...
                                <X className="w-4 h-4 cursor-pointer text-red-500" onClick={() => setFeatures(features.filter((_, i) => i !== idx))} />
                            </div>
                        ))}
                    </div>
                )}

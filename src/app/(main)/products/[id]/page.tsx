            ...
            <div className="container mx-auto px-4 py-8">
                <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 lg:p-10">
                        {/* Mobile thumbnails - TOP of the content for mobile */}
                        <div className="lg:hidden grid grid-cols-5 gap-2 mb-4">
                            {galleryImages.map((img, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => setSelectedImage(idx)}
                                    className={`aspect-square rounded-lg flex items-center justify-center border transition-all ${selectedImage === idx ? "border-blue-600 ring-2 ring-blue-100" : "border-gray-100 bg-white"
                        ...
                        {/* Left Column: Image Gallery (with vertical thumbnails on desktop) */}
                            ...
                            <div className="flex-1 w-full lg:max-w-[420px] mx-auto lg:mx-0">
                                ...
                                >
                                    {galleryImages[selectedImage].type === "placeholder" ? (
                                        ...
                                        <>
                                            <Image
                                                src={galleryImages[selectedImage].content}
                                                alt={product.title}
                                                fill
                                                className={`object-contain transition-opacity duration-300 ${showZoom ? 'opacity-0' : 'opacity-100'}`}
                                                unoptimized
                                            />
                                            {/* Overlay Zoom Magnifier */}
                                            {showZoom && (
                                                <div
                                                    className="absolute inset-0 pointer-events-none"
                                                    style={{
                                                        backgroundImage: `url(${galleryImages[selectedImage].content})`,
